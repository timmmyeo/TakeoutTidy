    ...
    for root, _, files in os.walk(root_dir):
        ...
        for filename in files:
            ...
            if (total_files % 500) == 0:
                ...
                print(f"Status: Hashing file {total_files}...", end='\r', file=sys.stdout, flush=True)
            ...
            if file_hash in keeper_map:
                ...
                is_existing_in_archive = os.path.basename(os.path.dirname(existing_keeper_path)).startswith(ARCHIVE_PREFIX)
...
def process_duplicates(root_dir, keeper_map, execution_mode):
    ...
    for root, _, files in os.walk(root_dir):
        ...
        folder_log_entries = [f"--- Log for Archive Folder: {archive_folder_name} ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')}) ---\n"]
        folder_log_entries.append(f"Execution Mode: {execution_mode.upper()}\n")
        folder_log_entries.append(f"Keeper priority: Files with matching HASH outside this archive folder.\n")
        ...
        for filename in files:
            ...
            if duplicate_hash in keeper_map:
                ...
                if keeper_path == full_path_duplicate:
                    folder_log_entries.append(f"KEEP (Archive Keeper): {full_path_duplicate}\n")
                    continue
                ...
                if execution_mode in ['symlink', 'delete-only']:
                    try:
                        ...
                        folder_log_entries.append(f"DELETE (Duplicate Media): {os.path.basename(full_path_duplicate)}\n")
                        acted_on_media = True
                    except Exception as e:
                    ...
                    folder_log_entries.append(f"LOG ONLY (Would Delete Media): {os.path.basename(full_path_duplicate)}\n")
                ...
                if os.path.exists(json_path_duplicate):
                    if execution_mode in ['symlink', 'delete-only']:
                        try:
                            ...
                            folder_log_entries.append(f"DELETE (Duplicate JSON): {os.path.basename(json_path_duplicate)}\n")
                        except Exception as e:
                            error_msg = f"ERROR deleting JSON {json_path_duplicate}: {e}"
                            folder_log_entries.append(error_msg + '\n')
                            print(f"Error in {archive_folder_name}: {error_msg}")
                    else: # log-only mode
                        folder_log_entries.append(f"LOG ONLY (Would Delete JSON): {os.path.basename(json_path_duplicate)}\n")
                ...
                if execution_mode == 'symlink' and acted_on_media:
                    try:
                        ...
                        folder_log_entries.append(f"LINKED (Replaced Duplicate): {os.path.basename(full_path_duplicate)} -> {keeper_path}\n")
                        total_replaced += 1
                        ...
                        if 'operation not permitted' in str(e).lower():
                            ...
                            print(error_msg)
                            sys.exit(1)
                        error_msg = f"ERROR creating symlink at {full_path_duplicate}: {e}"
                        folder_log_entries.append(error_msg + '\n')
                        print(f"Error in {archive_folder_name}: {error_msg}")
                elif execution_mode == 'delete-only' and acted_on_media:
                    folder_log_entries.append(f"DELETED (No Symlink Created): {os.path.basename(full_path_duplicate)} (Keeper: {keeper_path})\n")
                    total_replaced += 1
                    current_folder_replacements += 1 # Count as acted upon
                elif execution_mode == 'log-only':
                     folder_log_entries.append(f"LOG ONLY (Would Symlink): {os.path.basename(full_path_duplicate)} -> {keeper_path}\n")

            else:
                # File is unique (by hash)
                folder_log_entries.append(f"KEEP (Unique Archive File): {full_path_duplicate}\n")
        ...
        if current_folder_replacements > 0 or execution_mode == 'log-only':
            ...
            try:
                ...
                print(f"   -> {execution_mode.upper()} completed. Log saved to {FOLDER_LOG_FILENAME} inside the folder.")
            except Exception as e:
                print(f"   -> ERROR: Could not write log file in {archive_folder_name}: {e}")
        else:
...
def main():
    ...
    if EXECUTION_MODE != 'log-only':
        ...
        if EXECUTION_MODE == 'symlink':
            print("!!! WARNING: SYM-LINKING requires Administrator privileges on Windows/WSL. !!!")
        input(f"Press Enter to continue (Mode: {EXECUTION_MODE}) or Ctrl+C to cancel...")
    else:
