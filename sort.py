...
def run_exiftool(args):
    ...
    try:
        ...
        subprocess.run(['exiftool', *args], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        print(f"ExifTool error on arguments {args}: {e.stderr.decode().strip()}")
        return False
    except FileNotFoundError:
        print("Error: exiftool not found. Make sure it is installed and in your system's PATH.")
        exit(1)
    return True
...
def load_json_metadata_map(takeout_path):
    ...
    print(f"Successfully mapped {len(metadata_map)} unique metadata entries by 'title'.")
    return metadata_map
    ...
    if failures:
        ...
        with open(failure_log_path, 'w', encoding='utf-8') as f:
            for path in failures:
                f.write(path + '\n')
        print(f"**LOGGED:** {len(failures)} files failed the ExifTool merge. See **{failure_log_path}** for paths.")
    ...
    if untouched:
        ...
        with open(untouched_log_path, 'w', encoding='utf-8') as f:
            for path in untouched:
                f.write(path + '\n')
        print(f"**LOGGED:** {len(untouched)} files were skipped (no JSON found). See **{untouched_log_path}** for paths.")
...
if not os.path.isdir(TAKEout_DIRECTORY):
    ...
    print("Please update the 'TAKEout_DIRECTORY' variable in the script to the root of your extracted Google Photos folder.")
else:
    print("--- Starting Google Photos Metadata Merge ---")
    print("!!! WARNING: This script modifies files in place. ENSURE YOU HAVE A BACKUP COPY OF YOUR DATA !!!")
